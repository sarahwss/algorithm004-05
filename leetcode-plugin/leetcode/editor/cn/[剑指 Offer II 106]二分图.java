package leetcode.editor.cn;//存在一个 无向图 ，图中有 n 个节点。其中每个节点都有一个介于 0 到 n - 1 之间的唯一编号。
//
    ...
    private boolean setColor(int[][] graph, int[] colors, int i, int color) {
        ...
        while (!deque.isEmpty()) {
            ...
            for (int neighbor : graph[j]) {
                if (colors[neighbor] != -1 && colors[neighbor] == c) {
                    return false;
                }
                // neighbor
