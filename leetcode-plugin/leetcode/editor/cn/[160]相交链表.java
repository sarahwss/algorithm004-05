...
// Related Topics 哈希表 链表 双指针 👍 1987 👎 0
...
public class Solution {
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        ListNode tail = p.next == null ? p : q;
        // 短链头
        ListNode s = tail == p ? headA : headB;
        // 长链头
        ListNode l = tail == p ? headB : headA;
        // 长链表继续遍历尾节点
        while (p.next != null) {
            p = p.next;
        }
        ...
        do {
            p = p.next;
            q = q.next.next;
        } while (p != q); // 注意while放到后面，起始点相同
        // q即是交点
        p = s; // 短链头，注意不要直接赋值headA

        while (p != q) {
            p = p.next;
            q = q.next;
        }
        tail.next = null; // 恢复tail.next
        return p;
    }
}
